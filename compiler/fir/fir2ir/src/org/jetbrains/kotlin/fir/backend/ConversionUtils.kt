...
fun FirNamedReference.toFunctionSymbol(symbolTable: SymbolTable): IrFunctionSymbol? {
    if (this is FirResolvedCallableReference) {
        val callableSymbol = this.callableSymbol
        if (callableSymbol is FirFunctionSymbol) {
            return callableSymbol.toFunctionSymbol(symbolTable)
        }
    }
    return null
}
